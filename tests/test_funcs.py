...
def test_nx2gt(nx_graph):
    ...
    try:
        ...
        nodemap = {v: label for v, label in zip(gt_graph.vertices(), gt_graph.vp["node_label"], strict=False)}
    ...
    if not nx_graph.is_directed():
        assert set(
            [frozenset((nodemap[e.source()], nodemap[e.target()])) for e in gt_graph.edges()],
        ) == set([frozenset(e) for e in nx_graph.edges()])
    else:
        assert [(nodemap[e.source()], nodemap[e.target()]) for e in gt_graph.edges()] == list(
            nx_graph.edges(),
        )
...
def test_nx2gt_without_labels(nx_graph):
    ...
    nodemap = {v: i for v, i in zip(nx_graph.nodes(), range(len(nx_graph.nodes())), strict=False)}

    assert list(gt_graph.get_vertices()) == list(nodemap.values())
    if not nx_graph.is_directed():
        assert set([frozenset((e.source(), e.target())) for e in gt_graph.edges()]) == set(
            [frozenset((nodemap[u], nodemap[v])) for u, v in nx_graph.edges()],
        )
    else:
        assert [(e.source(), e.target()) for e in gt_graph.edges()] == list(
            (nodemap[u], nodemap[v]) for u, v in nx_graph.edges()
        )
...
def test_gt2nx(gt_graph):
    if "node_labels" in gt_graph.vp:
        ...
        nodemap = {
            v: label for v, label in zip(gt_graph.get_vertices(), gt_graph.vp["node_labels"], strict=False)
        }
...
def test_gt2igraph(gt_graph):
    if "node_labels" in gt_graph.vp:
        ...
        nodemap = {v: label for v, label in zip(gt_graph.vertices(), gt_graph.vp["node_labels"], strict=False)}
        ...
        if not gt_graph.is_directed():
            assert set([frozenset((e.source(), e.target())) for e in gt_graph.edges()]) == set(
                [frozenset(e.tuple) for e in ig_graph.es()],
            )
...
def test_round_robin_ig(ig_graph):
    ...
    assert set(ig_graph.vertex_attributes()).add("name") == set(
        reversed_ig_graph.vertex_attributes(),
    ).add("name")
