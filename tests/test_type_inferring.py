...
try:
    ...
    def test_numpy_type():
        assert infer_type(1.5, as_vector=False) == "double"

    def test_numpy_type_long():
        pyintergraph.USE_LONG_DOUBLE = True
        assert infer_type(1.5, as_vector=False) == "long double"
...
def test_raise_infer_exception():
    invalids = [["mein", "dein", 56, 4.3, "haben"], [2**63]]
    for invalid in invalids:
        with pytest.raises(pyintergraph.PyIntergraphInferError):
            infer_type(invalid)
